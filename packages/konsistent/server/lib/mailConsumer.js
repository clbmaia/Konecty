...
const smtpTransport = Npm.require('nodemailer-smtp-transport');
const emailTemplates = Npm.require('swig-email-templates');
const xoauth2 = Npm.require('xoauth2');
...
mailConsumer.sendEmail = function(record, cb) {
	...
	if (record.server != null) {
		if (transporters[record.server] != null) {
			...
			if (((record._user != null ? record._user.length : undefined) > 0) && (__guard__(namespace != null ? namespace.googleApp : undefined, x => x.clientId) != null) && (__guard__(namespace != null ? namespace.googleApp : undefined, x1 => x1.secret) != null)) {
				...
				console.log('IF -> user?.services?.google?.idToken ->',__guard__(__guard__(user != null ? user.services : undefined, x3 => x3.google), x2 => x2.idToken));
				if (__guard__(__guard__(user != null ? user.services : undefined, x5 => x5.google), x4 => x4.idToken) != null) {
					...
					if (__guard__(__guard__(user != null ? user.services : undefined, x7 => x7.google), x6 => x6.idToken) != null) {
						console.log('GENERATOR -> user?.services?.google?.idToken ->',__guard__(user != null ? user.services : undefined, x8 => x8.google));
	...
	if (!record.to) {
		...
		if (mail.to) {
			var serverHost = __guard__(__guard__(server != null ? server.transporter : undefined, x10 => x10.options), x9 => x9.host);
...
mailConsumer.consume = function() {
	...
	const findAndModify = Meteor.wrapAsync(_.bind(collection.findAndModify, collection));
	...
	const updatedRecords = findAndModify(query, sort, update, options);
	const records = [];
